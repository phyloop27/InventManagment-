...
public class MessagesTest {
    ...
    public void testLocalizedErrorMessages() throws Exception {
        ...
        assertEquals("Malformed database URL, failed to parse the ConnectionDB string near 'Test'.",
        ...
        assertEquals("Illegal database URL, in a 'Test' multi-host ConnectionDB it is required the same credentials in all hosts.",
                Messages.getString("ConnectionString.14", new Object[] { "Test" }));
        assertEquals("Illegal database URL, in a 'Test' multi-host ConnectionDB it is required that all or none of the hosts set a \"priority\" value.",
                Messages.getString("ConnectionString.15", new Object[] { "Test" }));
        assertEquals("Illegal database URL, in a 'Test' multi-host ConnectionDB the \"priority\" setting must be a value between 0 and 100.",
                Messages.getString("ConnectionString.16", new Object[] { "Test" }));

        assertEquals("Cannot load ConnectionDB class because of underlying exception: " + ex.toString(),
        ...
        assertEquals(
                "User does not have access to metadata required to determine stored procedure parameter types."
                        + " If rights can not be granted, configure ConnectionDB with \"noAccessToProcedureBodies=true\" "
        ...
        assertEquals("Can not call setNCharacterStream() when ConnectionDB character set isn't UTF-8", Messages.getString("ServerPreparedStatement.28"));
        assertEquals("Can not call setNClob() when ConnectionDB character set isn't UTF-8", Messages.getString("ServerPreparedStatement.29"));
        assertEquals("Can not call setNString() when ConnectionDB character set isn't UTF-8", Messages.getString("ServerPreparedStatement.30"));
