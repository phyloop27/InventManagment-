 ...
 * "lifecycle" methods on our ConnectionDB implementation.
 *
 * The driver will create one instance of a given interceptor per-ConnectionDB.
 */
public interface ConnectionLifecycleInterceptor {

    /**
     * Called once per ConnectionDB that wants to use the extension
     *
     * The properties are the same ones passed in in the URL or arguments to
     * Driver.connect() or DriverManager.getConnection().
     *
     * @param conn
     *            the ConnectionDB for which this extension is being created
     * @param props
