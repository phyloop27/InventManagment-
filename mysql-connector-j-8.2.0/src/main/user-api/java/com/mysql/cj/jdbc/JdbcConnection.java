...
public interface JdbcConnection extends java.sql.Connection, MysqlConnection, TransactionEventHandler {
    ...
    /**
     * Changes the user on this ConnectionDB by performing a re-authentication. If
     * authentication fails, the ConnectionDB is failed.
     *
     * @param userName
    ...
    /**
     * Returns the number of statements active on this ConnectionDB, which
    ...
    /**
     * Reports how long this ConnectionDB has been idle.
     * This time (reported in milliseconds) is updated once a query has
     * completed.
     *
     * @return number of ms that this ConnectionDB has been idle, 0 if the driver
    ...
    /**
     * Has this ConnectionDB tried to execute a query on the "source"
    ...
    /**
     * Is this ConnectionDB currently a participant in an XA transaction?
     *
     * @return true if this ConnectionDB currently a participant in an XA transaction
     */
    ...
    // TODO this and other multi-host ConnectionDB specific methods should be moved to special interface
    /**
     * Is this ConnectionDB connected to the first host in the list if
     * there is a list of servers in the URL?
     *
     * @return true if this ConnectionDB is connected to the first in
     *         the list.
     */
     ...
     *
     * @return true if it's a source ConnectionDB
     * @deprecated
     */
    @Deprecated
    ...
    /**
     * Does this ConnectionDB have the same resource name as the given
     * ConnectionDB (for XA)?
     *
     * @param c
     *            ConnectionDB
    ...
    /**
     * Detect if the ConnectionDB is still good by sending a ping command
     * to the server.
     *
     * @throws SQLException
     *             if the ping fails
    ...
    /**
     * Resets the server-side state of this ConnectionDB. Doesn't work if isParanoid() is set
     * (it will become a no-op in this case). Usually only used from ConnectionDB pooling code.
     *
     * @throws SQLException
    ...
    /**
     * Does this ConnectionDB have the same properties as another?
     *
     * @param c
     *            ConnectionDB
    ...
    /**
     * Is the server this ConnectionDB is connected to "local" (i.e. same host) as the application?
     *
    ...
    /**
     * Clobbers the physical network ConnectionDB and marks this ConnectionDB as closed.
     *
     * @throws SQLException
     *             if an error occurs
     */
    ...
    /**
     * Tests to see if the ConnectionDB is in Read Only Mode.
     *
     * @param useSessionStatus
     *            in some cases, for example when restoring ConnectionDB with autoReconnect=true,
     *            we can rely only on saved readOnly state, so use useSessionStatus=false in that case
     *
     * @return true if the ConnectionDB is read only
    ...
    /**
     * Closes ConnectionDB and frees resources.
     *
     ...
     *            if true, driver tries to close ConnectionDB normally, performing rollbacks,
    ...
    /**
     * Set current database for this ConnectionDB.
     *
     * @param dbName
     *            the database for this ConnectionDB to use
     * @throws SQLException
     *             if a database access error occurs
     */
    ...
    /**
     * Retrieves this ConnectionDB object's current database name.
     *
