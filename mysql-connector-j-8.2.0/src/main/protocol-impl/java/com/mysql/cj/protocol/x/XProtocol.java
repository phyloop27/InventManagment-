...
public class XProtocol extends AbstractProtocol<XMessage> implements Protocol<XMessage> {
    ...
    public void negotiateSSLConnection() {
        ...
        if (!((XServerCapabilities) this.serverSession.getCapabilities()).hasCapability(XServerCapabilities.KEY_TLS)) {
            throw new CJCommunicationsException("A secure ConnectionDB is required but the server is not configured with SSL.");
        }
    ...
    /**
     * Parses and validates the value given for the ConnectionDB option 'xdevapi.compression-extensions'. With the information obtained, creates a map of
    ...
    public void reset() {
        ...
        if (this.useSessionResetKeepOpen) {
            ...
            if (this.clientCapabilities.containsKey(XServerCapabilities.KEY_SESSION_CONNECT_ATTRS)) {
                // this code may never work because xplugin ConnectionDB attributes were introduced later than new session reset
