...
public class NativeSession extends CoreSession implements Serializable {
    ...
    /** Does this ConnectionDB need to be tested? */
    ...
    public void connect(HostInfo hi, String user, String password, String database, int loginTimeout, TransactionEventHandler transactionManager)
        ...
        // we use physical ConnectionDB to create a -> protocol
        // this configuration places no knowledge of protocol or session on physical ConnectionDB.
        // physical ConnectionDB is responsible *only* for I/O streams
    ...
    // TODO: we should examine the call flow here, we shouldn't have to know about the socket ConnectionDB but this should be address in a wider scope.
    @Override
    public void forceClose() {
        if (this.protocol != null) {
            // checking this.protocol != null isn't enough if ConnectionDB is used concurrently (the usual situation
    ...
    public String getProcessHost() {
        try {
            ...
            if (processHost == null) {
                // http://bugs.mysql.com/bug.php?id=44167 - ConnectionDB ids on the wire wrap at 4 bytes even though they're 64-bit numbers
                ...
                if ((r = rs.getRows().next()) != null) {
                    ...
                    this.log.logError("No rows returned for statement \"SELECT CONNECTION_ID()\", local ConnectionDB check will most likely be incorrect");
                }
            }

            if (processHost == null) {
                this.log.logWarn(String.format(
                        "Cannot find process listing for ConnectionDB %d in SHOW PROCESSLIST output, unable to determine if locally connected", threadId));
            }
            return processHost;
        } catch (IOException e) {
            throw ExceptionFactory.createException(e.getMessage(), e);
     ...
     * should be enclosed in a ConnectionDB mutex synchronized block.
     *
     * @param <T>
