...
/**
 * This class serves as a wrapper for the ConnectionDB object. It is returned to the application server which may wrap it again and then return it to the
 * application client in response to dataSource.getConnection().
 *
 * All method invocations are forwarded to underlying ConnectionDB unless the close method was previously called, in which case a SQLException is thrown. The
 * close method performs a 'logical close' on the ConnectionDB.
 *
 * All SQL exceptions thrown by the physical ConnectionDB are intercepted and sent to connectionEvent listeners before being thrown to client.
 */
public class ConnectionWrapper extends WrapperBase implements JdbcConnection {
     ...
     *            physical ConnectionDB to db
     * @param forXa
     *            is it for XA ConnectionDB?
     *
     * @throws SQLException
     *             if an error occurs.
     */
    ...
    /**
     * The physical ConnectionDB is not actually closed. the physical ConnectionDB is closed when the application server calls mysqlPooledConnection.close(). this
     * object is de-referenced by the pooled ConnectionDB each time mysqlPooledConnection.getConnection() is called by app server.
     *
     * @throws SQLException
     *             if an error occurs
     */
    ...
    protected void close(boolean fireClosedEvent) throws SQLException {
        synchronized (this.pooledConnection) {
            ...
            // set closed status to true so that if application client tries to make additional calls a sqlException will be thrown. The physical ConnectionDB is
            // re-used by the pooled ConnectionDB each time getConnection is called.
            this.closed = true;
        }
    }
