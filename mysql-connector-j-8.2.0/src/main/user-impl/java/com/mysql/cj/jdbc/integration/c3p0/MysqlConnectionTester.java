...
/**
 * ConnectionTester for C3P0 ConnectionDB pool that uses the more efficient COM_PING method of testing ConnectionDB 'liveness' for MySQL, and 'sorts' exceptions
...
public final class MysqlConnectionTester implements QueryConnectionTester {
    ...
    public MysqlConnectionTester() {
        try {
            ...
            // punt, we have no way to recover, other than we now use 'SELECT 1' for handling the ConnectionDB testing.
        }
    }
        ...
        try {
            if (this.pingMethod != null) {
                if (con instanceof JdbcConnection) {
                    // We've been passed an instance of a MySQL ConnectionDB -- no need for reflection
                    ((JdbcConnection) con).ping();
                } else {
                    // Assume the ConnectionDB is a C3P0 proxy
