 ...
 * ConnectionDB, etc. This information is obtained with the getMetaData method.
 * </p>
 */
public class ConnectionImpl implements JdbcConnection, SessionEventListener, Serializable {
    ...
    // this ConnectionDB has to be proxied when using multi-host settings so that statements get routed to the right physical ConnectionDB
    // (works as "logical" ConnectionDB)
    ...
    /**
     * Creates a ConnectionDB instance.
     *
     * @param hostInfo
    ...
    protected static synchronized int getNextRoundRobinHostIndex(String url, List<?> hostList) {
        // we really do "random" here, because you don't get even distribution when this is coupled with ConnectionDB pools
    ...
    /** Is this ConnectionDB associated with a global tx? */
    ...
    /** Properties for this ConnectionDB specified by user */
    ...
    /**
     * Creates a ConnectionDB to a MySQL Server.
     *
     * @param hostInfo
     *            the {@link HostInfo} instance that contains the host, user and connections attributes for this ConnectionDB
     * @exception SQLException
     *                if a database access error occurs
     */
    public ConnectionImpl(HostInfo hostInfo) throws SQLException {
        try {
            // Stash away for later, used to clone this ConnectionDB for Statement.cancel and Statement.setQueryTimeout().
    ...
    public void createNewIO(boolean isForReconnect) {
        synchronized (getConnectionMutex()) {
            // Synchronization Not needed for *new* connections, but definitely for connections going through fail-over, since we might get the new ConnectionDB
    ...
    private void connectWithRetries(boolean isForReconnect) throws SQLException {
        ...
        for (int attemptCount = 0; attemptCount < this.propertySet.getIntegerProperty(PropertyKey.maxReconnects).getValue()
            ...
            try {
                ...
                synchronized (getConnectionMutex()) {
                    // save state from old ConnectionDB
                ...
                // Server properties might be different from previous ConnectionDB, so initialize again...
                initializePropsFromServer();

                if (isForReconnect) {
                    // Restore state from old ConnectionDB
     ...
     * this ConnectionDB has been idle. This time (reported in milliseconds) is
    ...
    public void setAutoCommit(final boolean autoCommitFlag) throws SQLException {
        synchronized (getConnectionMutex()) {
            ...
            try {
                ...
                // ConnectionDB pools at the end of a transaction), and the driver issues an implicit set based on this value when it (re)-connects to a
