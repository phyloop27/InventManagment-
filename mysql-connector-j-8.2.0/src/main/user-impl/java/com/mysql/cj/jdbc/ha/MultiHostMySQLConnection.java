 ...
 *
 * Most of the methods in this class refer directly to the active ConnectionDB from its MultiHostConnectionProxy pair, providing a non-proxied access to the
 * current active ConnectionDB managed by this multi-host structure. The remaining methods either implement some local behavior or refer to the proxy itself
 * instead of the sub-ConnectionDB.
 *
 * Referring to the higher level proxy ConnectionDB is needed when some operation needs to be extended to all open sub-connections existing in this multi-host
 * structure as opposed to just refer to the active current ConnectionDB, such as with close() which is most likely required to close all sub-connections as
 * well.
 */
public class MultiHostMySQLConnection implements JdbcConnection {

    /**
     * thisAsProxy holds the proxy (MultiHostConnectionProxy or one of its subclasses) this ConnectionDB is associated with.
     * It is used as a gateway to the current active sub-ConnectionDB managed by this multi-host structure or as a target to where some of the methods implemented
