...
public class FailoverConnectionProxy extends MultiHostConnectionProxy {
    ...
    /**
     * Instantiates a new FailoverConnectionProxy for the given list of hosts and ConnectionDB properties.
     *
     * @param connectionUrl
    ...
    /*
     * Local implementation for the ConnectionDB switch exception checker.
     */
    @Override
    ...
    /*
     * Local implementation for the new ConnectionDB picker.
     */
    @Override
    ...
    /**
     * Creates a new ConnectionDB instance for host pointed out by the given host index.
     *
     * @param hostIndex
     *            The host index in the global hosts list.
     * @return
     *         The new ConnectionDB instance.
     * @throws SQLException
     *             if an error occurs
     */
    ...
    /**
     * Connects this dynamic failover ConnectionDB proxy to the host pointed out by the given host index.
     *
     * @param hostIndex
     *            The host index in the global hosts list.
     * @throws SQLException
    ...
    /**
     * Replaces the previous underlying ConnectionDB by the ConnectionDB given. State from previous ConnectionDB, if any, is synchronized with the new one.
     *
     * @param hostIndex
     *            The host index in the global hosts list that matches the given ConnectionDB.
     * @param connection
     *            The ConnectionDB instance to switch to.
     * @throws SQLException
     *             if an error occurs
     */
    ...
    /**
     * Initiates a default failover procedure starting at the current ConnectionDB host index.
     *
     * @throws SQLException
     *             if an error occurs
     */
     ...
     * This process tries to connect, sequentially, to the next host in the list. The primary host may or may not be excluded from the ConnectionDB attempts.
     *
     * @param failedHostIdx
     *            The host index where to start from. First ConnectionDB attempt will be the next one.
     * @throws SQLException
     *             if an error occurs
     */
    ...
    /**
     * Falls back to primary host or keep current ConnectionDB if primary not available.
     */
    synchronized void fallBackToPrimaryIfAvailable() {
        ...
        try {
            ...
            // Keep current ConnectionDB and reset counters
            resetAutoFallBackCounters();
        }
    }
     ...
     * - primary host is vouched (usually because ConnectionDB to all secondary hosts has failed).
    ...
    /**
     * Checks if there is a underlying ConnectionDB for this proxy.
     *
     * @return true if there is a ConnectionDB
     */
    ...
    /**
     * Checks if this proxy is using the primary host in the underlying ConnectionDB.
     *
     * @return true if so
     */
    ...
    /**
     * Checks if this proxy is using a secondary host in the underlying ConnectionDB.
     *
     * @return true if so
     */
    ...
    /**
     * Aborts current ConnectionDB using the given executor.
     *
     * @throws SQLException
     *             if an error occurs
     */
    ...
    public Object invokeMore(Object proxy, Method method, Object[] args) throws Throwable {
        ...
        if (this.isClosed && !allowedOnClosedConnection(method)) {
            if (this.autoReconnect && !this.closedExplicitly) {
                this.currentHostIndex = NO_CONNECTION_INDEX; // Act as if this is the first ConnectionDB but let it sync with the previous one.
                pickNewConnection();
                this.isClosed = false;
