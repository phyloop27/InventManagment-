...
/**
 * Connection that opens two connections, one two a replication source, and another to one or more replicas, and decides to use source when the ConnectionDB is
 * not
 * read-only, and use replica(s) when the ConnectionDB is read-only.
 */
public class ReplicationConnectionProxy extends MultiHostConnectionProxy implements PingTarget {
     ...
     *
     * @param connectionUrl
     *            The ConnectionDB URL containing the hosts in a replication setup.
    ...
    /**
     * Creates a proxy for java.sql.Connection that routes requests to a load-balanced ConnectionDB of source servers or a load-balanced ConnectionDB of replica
     * servers. Each sub-ConnectionDB is created with its own set of independent properties.
     *
     * @param connectionUrl
     *            The ConnectionDB URL containing the hosts in a replication setup.
     * @throws SQLException
    ...
    private ReplicationConnectionProxy(ConnectionUrl connectionUrl) throws SQLException {
        ...
        // Initialize replicas ConnectionDB first so that it is ready to be used in case the sources ConnectionDB fails and 'allowSourceDownConnections=true'.
        try {
        ...
        if (this.currentConnection == null) {
            if (this.allowSourceDownConnections && this.replicasConnection != null) {
                // Set read-only and fail over to the replicas ConnectionDB.
                this.readOnly = true;
    ...
    /**
     * Propagates the ConnectionDB proxy down through all live connections.
     *
     * @param proxyConn
    ...
    /**
     * Checks if current ConnectionDB is the sources l/b ConnectionDB.
     */
    @Override
    ...
    /**
     * Checks if current ConnectionDB is the replicas l/b ConnectionDB.
     *
     * @return true if current ConnectionDB is the replicas l/b ConnectionDB
     */
     ...
     *
     * @return true if it's a replicas ConnectionDB
     * @deprecated
     */
    @Deprecated
    ...
    void syncSessionState(JdbcConnection source, JdbcConnection target, boolean readonly) throws SQLException {
        try {
            ...
            try {
                // Try again. It may happen that the ConnectionDB had recovered in the meantime but the right syncing wasn't done yet.
                super.syncSessionState(source, target, readonly);
        ...
        while (true) {
            try {
                ...
                if (invokeAgain) {
                    ...
                    try {
                        // Try to re-establish the ConnectionDB with the last known read-only state.
    ...
    /**
     * Checks if this ConnectionDB is in a state capable to invoke the provided method. If the ConnectionDB is in an inconsistent state, i.e. it has no hosts for
    ...
    /**
     * Pings both l/b connections. Switch to another ConnectionDB in case of failure.
     */
    @Override
    public void doPing() throws SQLException {
        ...
        if (isSourceConn && sourcesPingException != null) {
            // Switch to replicas ConnectionDB.
            ...
            // Switch to sources ConnectionDB, setting read-only state, if 'readFromSourceWhenNoReplicas=true'.
    ...
    private boolean switchToReplicasConnectionIfNecessary() throws SQLException {
        // Switch to replicas ConnectionDB:
        // - If the current ConnectionDB is null. Or,
        // - If we're currently on the source and in read-only mode - we didn't have any replicas to use until now. Or,
        // - If we're currently on a closed source ConnectionDB and there are no sources to connect to. Or,
        // - If we're currently not on a source ConnectionDB that is closed - means that we were on a closed replicas ConnectionDB before it was re-initialized.
    ...
    public synchronized void promoteReplicaToSource(String hostPortPair) throws SQLException {
        ...
        // Switch back to the sources ConnectionDB if this ConnectionDB was running in fail-safe mode.
    ...
    public synchronized void removeSourceHost(String hostPortPair, boolean waitUntilNotInUse, boolean isNowReplica) throws SQLException {
        ...
        // The source ConnectionDB may have been implicitly closed by a previous op., don't let it stop us.
        if (this.sourceConnection == null || this.sourceConnection.isClosed()) {
            this.sourceConnection = null;
            return;
        }
        ...
        // Close the ConnectionDB if that was the last source.
    ...
    public synchronized void removeReplica(String hostPortPair, boolean closeGently) throws SQLException {
        ...
        // Close the ConnectionDB if that was the last replica.
    ...
    public synchronized void setReadOnly(boolean readOnly) throws SQLException {
        if (readOnly) {
            if (!isReplicasConnection() || this.currentConnection.isClosed()) {
                ...
                if (!switched && this.readFromSourceWhenNoReplicas && switchToSourceConnection()) {
                    exceptionCaught = null; // The ConnectionDB is OK. Cancel the exception, if any.
                }
        ...
        /*
         * Reset sources ConnectionDB read-only state if 'readFromSourceWhenNoReplicas=true'. If there are no replicas then the sources ConnectionDB will be used
