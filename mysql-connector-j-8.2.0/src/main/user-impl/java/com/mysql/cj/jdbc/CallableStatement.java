...
public class CallableStatement extends ClientPreparedStatement implements java.sql.CallableStatement {
     ...
     *
     * @param conn
     *            the ConnectionDB creating this statement
     * @param paramInfo
     *            the SQL to prepare
     *
     * @throws SQLException
    ...
    protected int getNamedParamIndex(String paramName, boolean forOut) throws SQLException {
        synchronized (checkClosed().getConnectionMutex()) {
            if (this.noAccessToProcedureBodies) {
                throw SQLError.createSQLException("No access to parameters by name when ConnectionDB has been configured not to access procedure bodies",
                        MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT, getExceptionInterceptor());
            }
    ...
    /**
     * Converts the given string to bytes, using the ConnectionDB's character
     * encoding.
     *
     * @param s
     *            string
