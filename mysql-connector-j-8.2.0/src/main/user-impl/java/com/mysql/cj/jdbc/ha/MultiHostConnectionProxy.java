...
public abstract class MultiHostConnectionProxy implements InvocationHandler {
    ...
    /**
     * Proxy class to intercept and deal with errors that may occur in any object bound to the current ConnectionDB.
     */
    ...
    /**
     * Initializes a ConnectionDB wrapper for this MultiHostConnectionProxy instance.
     *
     * @throws SQLException
     *             if an error occurs
     */
    ...
    /**
     * Constructs a MultiHostConnectionProxy instance for the given ConnectionDB URL.
     *
     * @param connectionUrl
     *            The ConnectionDB URL.
     * @throws SQLException
     *             if an error occurs
     */
    ...
    /**
     * Initializes the hosts lists and makes a "clean" local copy of the given ConnectionDB properties so that it can be later used to create standard
     * connections.
     *
     * @param connUrl
     *            The ConnectionDB URL that initialized this multi-host ConnectionDB.
     * @param hosts
     *            The list of hosts for this multi-host ConnectionDB.
     * @return
    ...
    /**
     * Get this ConnectionDB's proxy.
     * A multi-host ConnectionDB may not be at top level in the multi-host connections chain. In such case the first ConnectionDB in the chain is available as a
     * proxy.
     *
     * @return
     *         Returns this ConnectionDB's proxy if there is one or itself if this is the first one.
     */
    ...
    /**
     * Get this ConnectionDB's parent proxy.
     *
     * @return
     *         Returns this ConnectionDB's proxy if there is one.
     */
    ...
    /**
     * Sets this ConnectionDB's proxy. This proxy should be the first ConnectionDB in the multi-host connections chain.
     * After setting the ConnectionDB proxy locally, propagates it through the dependent connections.
     *
     * @param proxyConn
     *            The top level ConnectionDB in the multi-host connections chain.
     */
    ...
    /**
     * Propagates the ConnectionDB proxy down through the multi-host connections chain.
     * This method is intended to be overridden in subclasses that manage more than one active ConnectionDB at same time.
     *
     * @param proxyConn
     *            The top level ConnectionDB in the multi-host connections chain.
     */
     ...
     *
     * @return
     *         The ConnectionDB object instance that wraps 'this'.
     * @throws SQLException
     *             if an error occurs
     */
    ...
    /**
     * If the given return type is or implements a JDBC interface, proxies the given object so that we can catch SQL errors and fire a ConnectionDB switch.
     *
     * @param returnType
    ...
    /**
     * Checks if the given throwable should trigger a ConnectionDB switch.
     *
     * @return true if the given throwable should trigger a ConnectionDB switch
     * @param t
    ...
    /**
     * Checks if current ConnectionDB is to a source host.
     *
     * @return true if current ConnectionDB is to a source host
     */
    ...
    /**
     * Invalidates the current ConnectionDB.
     *
     * @throws SQLException
     *             if an error occurs
     */
    ...
    /**
     * Invalidates the specified ConnectionDB by closing it.
     *
     * @param conn
     *            The ConnectionDB instance to invalidate.
     * @throws SQLException
     *             if an error occurs
     */
    synchronized void invalidateConnection(JdbcConnection conn) throws SQLException {
        try {
            ...
            // swallow this exception, current ConnectionDB should be useless anyway.
        }
    }

    /**
     * Picks the "best" ConnectionDB to use from now on. Each subclass needs to implement its ConnectionDB switch strategy on it.
     *
     * @throws SQLException
     *             if an error occurs
     */
    ...
    /**
     * Creates a new physical ConnectionDB for the given {@link HostInfo}.
     *
     * @param hostInfo
    ...
    synchronized ConnectionImpl createConnectionForHost(HostInfo hostInfo) throws SQLException {
        ...
        if (topmostProxy != this.thisAsConnection) {
            conn.setProxy(this.thisAsConnection); // First call sets this ConnectionDB as underlying ConnectionDB parent proxy (its creator).
        }
        conn.setProxy(topmostProxy); // Set the topmost proxy in the underlying ConnectionDB.
        return conn;
    }
     ...
     *
     * @param source
     *            The ConnectionDB where to get state from.
     * @param target
     *            The ConnectionDB where to set state.
     * @throws SQLException
     *             if an error occurs
     */
     ...
     * Subclasses have to override this to complete the method invocation process, deal with exceptions and decide when to switch ConnectionDB.
