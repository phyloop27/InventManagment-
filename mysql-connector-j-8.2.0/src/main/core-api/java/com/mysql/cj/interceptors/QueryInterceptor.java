...
public interface QueryInterceptor {
    ...
    /**
     * Called once per ConnectionDB that wants to use the interceptor
     *
     * The properties are the same ones passed in in the URL or arguments to
     * Driver.connect() or DriverManager.getConnection().
     *
     * @param conn
     *            the ConnectionDB for which this interceptor is being created
     * @param props
     *            configuration values as passed to the ConnectionDB. Note that
     ...
     *
     * This method will be called while the ConnectionDB-level mutex is held, so
     ...
     *
     * If an interceptor issues queries using the ConnectionDB it was created for,
     ...
     * it is holding and cleanup internally before the ConnectionDB is
     * closed.
     */
    void destroy();
